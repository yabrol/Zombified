    ...
    public void renderDialogs(Graphics g)
    {
        ...
        for (Sprite s : dialogSprites)
        {
            ...
            if(s.getSpriteType().getSpriteTypeID().equals(STATS_DIALOG_TYPE) && s.getState().equals(VISIBLE_STATE))
            {
              ...
              MahjongSolitaireRecord rec = ((MahjongSolitaireMiniGame)game).getFileManager().loadRecord();
              stats = level;
              ...
              {
                stats = "Fastest Win: " + ((MahjongSolitaireDataModel)((MahjongSolitaireMiniGame)game).getDataModel()).timeToText(rec.getFastestTime(level));
              }
              else
                stats = "Fastest Win: -";
